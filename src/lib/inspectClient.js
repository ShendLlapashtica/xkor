// Browser-side Encar inspection fetcher — bypasses Vercel IP blocks via user's browser IP
...
export async function fetchInspect(id) {
  ...
  for (const url of urls) {
    ...
    attempts.push(
      fetch(`https://api.allorigins.win/get?url=${enc}`, { signal: sig })
        .then(r => r.ok ? r.json() : null)
        .then(o => (o?.contents ? JSON.parse(o.contents) : null))
        .catch(() => null)
    );
    attempts.push(
    ...
    attempts.push(
      fetch(`https://api.codetabs.com/v1/proxy?quest=${enc}`, { signal: sig })
        .then(r => r.ok ? r.json() : null)
        .catch(() => null)
    );
  }
  ...
  let raw = null;
  try { raw = await Promise.any(attempts.map(nonNull)); } catch { raw = null; }
